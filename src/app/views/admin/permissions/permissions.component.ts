...
export class PermissionsComponent implements OnInit {
  ...
  savePermission() {
    if (this.selectedPermission.id) {
      // update existing
      const index = this.permissions.findIndex(s => s.id === this.selectedPermission.id);
      if (index !== -1) {
        this.permissions[index] = { ...this.selectedPermission };
      }
    } else {
      // add new
      const newId = this.permissions.length ? Math.max(...this.permissions.map(s => s.id)) + 1 : 1;
      this.selectedPermission.id = newId;
